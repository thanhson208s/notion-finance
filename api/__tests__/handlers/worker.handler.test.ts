...
import { handleWorkerRequest, processTelegramUpdate } from '../../_handlers/worker.handler'
...
vi.mock('../../_lib/connector', async () => {
  const actual = await vi.importActual<typeof import('../../_lib/connector')>('../../_lib/connector')
  return {
    ...actual,
    Connector: vi.fn(function Connector() {
...
vi.mock('../../_lib/cache', () => ({
  acquireWebhookJobLock: vi.fn(),
  getWebhookJobStatus: vi.fn(),
  releaseWebhookJobLock: vi.fn(),
  setWebhookJobStatus: vi.fn(),
}))
...
const getStateMock = vi.mocked(getWebhookJobStatus)
const acquireLockMock = vi.mocked(acquireWebhookJobLock)
const releaseLockMock = vi.mocked(releaseWebhookJobLock)
const setStateMock = vi.mocked(setWebhookJobStatus)
...
describe('processTelegramUpdateCore()', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('rejects System categories', async () => {
    ...
    await expect(processTelegramUpdate(makeUpdate(106), connector))
      .rejects.toThrow('Category "Transfer" is a System category and cannot be logged here')
    expect(sendMock).not.toHaveBeenCalled()
  })
  ...
  it('throws when inference fails', async () => {
    ...
    const connector = makeConnector([expenseCat])
    await expect(processTelegramUpdate(makeUpdate(107), connector))
  ...
  it('reply edit amount reuses updateTransaction balance reconciliation and edits the confirmation', async () => {
    ...
    expect(sendMock).toHaveBeenCalledTimes(1)
    expect(sendMock).toHaveBeenCalledWith(expect.stringContaining('<b>✏️ Updated</b>'), expect.objectContaining({ parseMode: 'HTML', replyToMessageId: 116 }))
  })
  ...
  it('reply edit rejects parent, System categories', async () => {
    ...
    await expect(processTelegramUpdate(replyUpdate, connector))
      .rejects.toThrow('Category "Food" is a parent category and cannot be selected')
    expect(connector.updateTransactionPage).not.toHaveBeenCalled()
    ...
    await expect(processTelegramUpdate(replyUpdate, connector))
      .rejects.toThrow('Category "Transfer" is a System category and cannot be selected')
    expect(connector.updateTransactionPage).not.toHaveBeenCalled()
  })
  ...
  it('reply edit rejects invalid timestamps', async () => {
    ...
    const connector = makeConnector([expenseCat])
    await expect(processTelegramUpdate(makeUpdate(118, {
    ...
    }), connector)).rejects.toThrow('Inferred timestamp invalid')
    expect(connector.updateTransactionPage).not.toHaveBeenCalled()
    expect(sendMock).not.toHaveBeenCalled()
  })
  ...
  it('reply delete archives the transaction and tombstones the original message without a Tx anchor', async () => {
    ...
    expect(sendMock).toHaveBeenCalledTimes(1)
    expect(sendMock).toHaveBeenCalledWith(expect.stringContaining('<b>🗑 Deleted</b>'), expect.objectContaining({ parseMode: 'HTML', replyToMessageId: 119 }))
...
const workerUpdate: TelegramUpdate = {
  update_id: 321,
  message: {
    message_id: 456,
    chat: { id: 42 },
...
function makeWorkerRequest(headers: Record<string, string> = {}) {
  return {
    method: 'POST',
    headers: {
      'x-qstash-worker-secret': 'worker-secret',
      ...headers,
    },
    body: workerUpdate,
  }
...
describe('handleWorkerRequest()', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.stubEnv('QSTASH_WORKER_SECRET', 'worker-secret')
    vi.stubEnv('QSTASH_WORKER_RETRIES', '4')
    ...
    connectorMock.fetchAllAccounts.mockResolvedValue([makeAccount(200)])
    connectorMock.fetchAccount.mockResolvedValue(makeAccount(200))
    connectorMock.fetchCategories.mockResolvedValue([expenseCat])
    connectorMock.fetchCategory.mockResolvedValue(expenseCat)
    connectorMock.fetchAllCards.mockResolvedValue([])
    connectorMock.addExpense.mockResolvedValue(makeTx(50))
    connectorMock.addIncome.mockResolvedValue(makeTx(50, {
      fromAccountId: undefined,
      toAccountId: 'acc-1',
    }))
    connectorMock.updateAccountAfterTransaction.mockResolvedValue(makeAccount(150))
    inferMock.mockResolvedValue(inferred())
  })

  it('skips already-terminal updates', async () => {
    getStateMock.mockResolvedValue('succeeded')
    const res = makeResponse()

    await handleWorkerRequest(makeWorkerRequest() as never, res as never)
  ...
  it('skips duplicate concurrent deliveries when a lock exists', async () => {
  ...
  it('stores terminal state after successful processing', async () => {
    const res = makeResponse()

    await handleWorkerRequest(makeWorkerRequest() as never, res as never)

    expect(acquireLockMock).toHaveBeenCalledWith(321)
    expect(setStateMock).toHaveBeenCalledWith(321, 'succeeded')
    expect(releaseLockMock).toHaveBeenCalledWith(321)
    expect(res.statusCode).toBe(200)
  })

  it('returns 500 for transient failures before the final attempt without notifying Telegram', async () => {
  ...
  it('sends one failure reply and stores failed state on the final transient attempt', async () => {
    ...
    await handleWorkerRequest(makeWorkerRequest({ 'upstash-retried': '4' }) as never, res as never)

    expect(sendMock).toHaveBeenCalledOnce()
    expect(sendMock).toHaveBeenCalledWith(
      expect.stringContaining('Gemini unavailable'),
      expect.objectContaining({ replyToMessageId: 456 })
    )
    expect(setStateMock).toHaveBeenCalledWith(321, 'failed')
    expect(res.statusCode).toBe(200)
  })

  it('sends one failure reply and stores failed state for permanent QueryError', async () => {
    connectorMock.fetchCategories.mockRejectedValue(new QueryError('Inferred category not found'))
    const res = makeResponse()
    ...
    expect(sendMock).toHaveBeenCalledOnce()
    expect(sendMock).toHaveBeenCalledWith(
      expect.stringContaining('Inferred category not found'),
      expect.objectContaining({ replyToMessageId: 456 })
    )
    expect(setStateMock).toHaveBeenCalledWith(321, 'failed')
    expect(res.statusCode).toBe(200)
